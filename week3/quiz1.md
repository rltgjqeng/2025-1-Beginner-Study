...
- 답: 새로운 브랜치와 기존 브랜치를 공통으로 하는 새로운 커밋을 생성하고 기존 브랜치로 병합됨, 새로운 브랜치의 커밋을 하나로 합친 뒤 기존 브랜치에 하나의 커밋으로 병합함. 새로운 브랜치에 있는 커밋 모두의 base를 재설정하여 새 커밋으로 기존 브랜치에 위치하도록 변경됨. 이때, 커밋 해쉬값이 변경되므로 주의.
...
- 답: 현재 작업 중인 위치는 develop 브랜치이다. 이 브랜치의 가장 최근 커밋의 id는 a1s2d3f이고 커밋 메세지는 'docs: readme 추가'이다.
